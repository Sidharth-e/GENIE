...
export function ThreadList() {
  const { threads, deleteThread, refetchThreads } = useThreads();
  ...
  const saveRename = async () => {
    ...
    try {
      await fetch("/api/agent/threads", {
        ...
        body: JSON.stringify({
          id: renamingId,
          title: renameValue || "Untitled thread",
        }),
      });
    ...
    <nav className="flex h-full flex-col border-r border-gray-200 bg-white/60 backdrop-blur-sm dark:border-gray-800 dark:bg-gray-900/60">
      <div className="flex items-center gap-2 px-3 pb-2 pt-2">
        ...
        <button
          ...
          className="border-border text-muted-foreground hover:bg-accent hover:text-accent-foreground inline-flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-md border"
          title="Refresh"
        >
          {refreshing ? (
