...
export function useChatThread({
  ...
  const {
    ...
    enabled: !!threadId,
    queryFn: () =>
      threadId ? fetchMessageHistory(threadId) : Promise.resolve([]),
    retry: false,
  });
  ...
  const {
    ...
    enabled: !!threadId,
    queryFn: () =>
      threadId ? fetchThread(threadId) : Promise.resolve(null as any),
    retry: false,
  });
  ...
  const handleStreamResponse = useCallback(
    async (streamParams: {
      ...
      try {
        ...
        stream.onmessage = (event: MessageEvent) => {
          try {
            ...
            if (
              ...
              const currentData = currentMessageRef.current
                .data as AIMessageData;
              const newContent =
        ...
        stream.addEventListener("title_generated", (event: MessageEvent) => {
          try {
    ...
    async (text: string, opts?: MessageOptions) => {
      ...
      const userMessage: MessageResponse = {
        ...
        data: {
          id: tempId,
          content: text,
          // Include document IDs for attachment display
          additional_kwargs: opts?.documentIds?.length
